...
@section('content-header', 'Purchase Invoice')
@section('css')
<style>
  .bg-gradient-primary {
...
<div class="invoice p-3" id="printThis">
  ...
  <div class="row invoice-info text-center">
    ...
    <div class="col-sm-4 invoice-col">
      To
      <address>
        <strong>{{ $products[0]->first_name }} {{ $products[0]->last_name }}</strong><br>
        <ul class="m-0 p-0 list-unstyled">
          <li>{{ $products[0]->address }}</li>
        </ul>
        Phone: {{ $products[0]->phone }}<br>
        Email: {{ $products[0]->email }}
      </address>
    </div>
    <!-- /.col -->
    <div class="col-sm-4 invoice-col">
      <b>{{$products[0]->invoice_number}}</b>
      <br>
      <b>Purchase Date:</b>{{$products[0]->purchaseDate}}<br>
      <b>Account:</b> {{$products[0]->acc_no}}
    </div>
    <!-- /.col -->
  </div>
  <hr>
      ...
      <table class="table table-sm" style="border-style:dashed">
        ...
        @foreach ($products as $product)
          <tr>
            ...
            <td>{{ $product->kg }}</td>
            <td>{{ $product->short >0 ? $product->short :0 }}</td>
            <td>Rs {{$product->tax>0?$product->tax : 0}}</td>
            <td>Rs {{ $product->price }}</td>
            <td>Rs {{ $product->quantity>0 ? number_format($product->purchase_price,2): 00}}</td>
          </tr>
          @endforeach
        </tbody>
      </table>
        ...
        <table class="table" id="dataTable">
          <tbody>
            <tr>
              ...
              <td>{{$products[0]->allTotal}}</td>
            </tr>
            <tr>
              <th>Discount</th>
              <td class="">Rs {{$products[0]->discount}}</td>
            </tr>
            <tr>
              <th>Paid Amount</th>
              <td>Rs {{$products[0]->paidAmount}}</td> 
            </tr>
            <tr>
              <th>Dues</th>
              <td>Rs <span id="dues">{{$products[0]->dues}}</span></td>
            </tr>
            <tr>
              <th>Payment Type</th>
              <td>{{$products[0]->payment_method}}</td>
            </tr>
            <tr>
              <th>Account Number</th>
              <td class="">{{$products[0]->acc_no}}</td>
            </tr>
