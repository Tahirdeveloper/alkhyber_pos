...
<a href="{{route('purchase.create')}}"><button type="button" class="btn btn-primary">
        ...
        <table class="table table-sm" id="dataTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td id="supplier">{{$supplier->first_name}} {{$supplier->last_name}}</td>
                    <td>{{$product->purchaseDate}}</td>
                    <td>{{$product->discount}}</td>
                    <td>{{$product->dues}}</td>
                    <td>{{$product->paidAmount}}</td>
                    <td>{{$product->allTotal}}</td>
                    @php
                    ...
                    if ($totalAmount < $allTotal) { $status='Partial' ; } elseif ($totalAmount==$allTotal) { $status='Paid' ; } elseif ($totalAmount> $allTotal) {
                        ...
                        @endphp
                        <td>
                            <span class="right badge badge-{{ $status === 'Paid' ? 'success' : ($status === 'Partial' ? 'warning' : ($status=='Unpaid' ? 'danger':'primary') )}}">
                                {{ $status }}
                            </span>
                        </td>
                        <td>
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu mx-n5" role="menu" style="left:-40 !important">
                                    <a href="{{route('purchase.edit',['id'=>$product->id])}}" class="dropdown-item">Edit</a>
                                    <a href="{{route('purchase.invoice',['id'=>$product->id])}}" class="dropdown-item">Invoice</a>
                                    <a class="dropdown-item" href="#" onclick="handleRefundClick({{ $product->id }})">Refund</a>
                                    <a class="dropdown-item" href="#" onclick="handlePayClick({{ $product->id }})">Pay</a>
                                    <a class="dropdown-item" href="#" onclick="handleDeleteClick({{ $product->id }})">Delete</a>
